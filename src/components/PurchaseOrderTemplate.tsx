...
export const PurchaseOrderTemplate = forwardRef<HTMLDivElement, PurchaseOrderTemplateProps>(
    ({ po, vendor, items, company, termsContent = '' }, ref) => {
        ...
        // Get clean notes without Subject: prefix
        const getCleanNotes = () => {
            if (!po.notes) return '';
            return po.notes.replace(/^Subject:\s*[^\n]*\n?/, '').trim();
        };
        ...
        return (
            <div ref={ref} className="bg-white text-black font-sans text-sm mx-auto" style={{ width: '210mm' }}>
                ...
                <div className="p-8 print-page" style={{ width: '210mm', minHeight: '297mm', display: 'flex', flexDirection: 'column', pageBreakBefore: 'always', breakBefore: 'page' }}>
                    <div>
                        {/* Terms and Conditions - Full page, no truncation */}
                        {termsContent ? (
                            <div className="text-xs">
                        ...
                        {getCleanNotes() && (
                            <div className="mt-6 text-xs">
                                <p className="font-bold mb-2 text-sm text-gray-600">Notes:</p>
                                <p className="text-gray-500 whitespace-pre-line">{getCleanNotes()}</p>
                            </div>
                        )}
                    </div>
